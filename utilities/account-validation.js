...
validate.updateAccountRules = () => {
  return [
    ...
    // valid email is required and cannot already exist in the DB (unless it's the current email)
    body("account_email")
      .trim()
      .escape()
      .notEmpty()
      ...
      .custom(async (account_email, { req }) => {
        const emailExists = await accountModel.checkExistingEmail(account_email)
        if (emailExists && parseInt(req.body.account_id) !== emailExists.account_id) {
...
validate.checkChangePasswordData = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    let nav = await utilities.getNav()
    // Need to get current account data to display in the form
